...
-(void)onTimer {
    [self checkCollision];
        ...
        enemy.center = CGPointMake(enemy.center.x, enemy.center.y+posgiancarlo.y);
    ...
    if (Number >= 5) {
        ...
        nick.center = CGPointMake(nick.center.x, nick.center.y+posnick.y);
    ...
    if (Number >= 10) {
        ...
        danny.center = CGPointMake(danny.center.x, danny.center.y+posdanny.y);
    ...
    if (Number >= 15) {
        ...
        jake.center = CGPointMake(jake.center.x, jake.center.y+posjake.y);
    ...
    if (Number >= 20) {
        ...
        blake.center = CGPointMake(blake.center.x, blake.center.y+posblake.y);
    ...
    if (Number >= 25) {
        ...
        diego.center = CGPointMake(diego.center.x, diego.center.y+posdiego.y);
    ...
    if (Number >= 30) {
        ...
        */
        ...
        if (kevin.center.x > 230 || kevin.center.x < 30)
            poskevin.x = -poskevin.x;
        if (kevin.center.y > 450 || kevin.center.y < 30)
...
-(void)checkCollision {
    if(CGRectIntersectsRect(player.frame, enemy.frame) || (CGRectIntersectsRect(player.frame, nick.frame) && (nick.hidden == NO)) || (CGRectIntersectsRect(player.frame, danny.frame) && (danny.hidden == NO)) || (CGRectIntersectsRect(player.frame, jake.frame) && (jake.hidden == NO)) || (CGRectIntersectsRect(player.frame, blake.frame) && (blake.hidden == NO)) || (CGRectIntersectsRect(player.frame, diego.frame) && (diego.hidden == NO)) || (CGRectIntersectsRect(player.frame, kevin.frame) && (kevin.hidden == NO)))
    {
        ...
        if (Number < 5) {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"I see victory has defeated you" message:[NSString stringWithFormat:@"Your Score: %i", Number] delegate:nil cancelButtonTitle:@"Mr. Wayne..." otherButtonTitles:nil];
            [alert show];
            //[alert release];
        } else if (Number < 10) {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Aahh, theatricality and deception..." message:[NSString stringWithFormat:@"Your Score: %i", Number] delegate:nil cancelButtonTitle:@"But we are initiated, aren't we?" otherButtonTitles:nil];
            [alert show];
        } else if (Number < 15) {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"You think the darkness is your ally?" message:[NSString stringWithFormat:@"Your Score: %i", Number] delegate:nil cancelButtonTitle:@"I was born in it" otherButtonTitles:nil];
            [alert show];
        } else if (Number < 45) {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"I didn't see the light until" message:[NSString stringWithFormat:@"Your Score: %i", Number] delegate:nil cancelButtonTitle:@"I was already a man" otherButtonTitles:nil];
            [alert show];
        } else if (Number < 65) {
